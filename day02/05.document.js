...
promise
  .then(() => {
    ...
    /*
      document.id  文档对象的id
      document.save()  将文档对象保存在数据库中
      document.isNew 文档对象是否是新的
      doument.update(更新的内容, 回调函数)
     */
    Students.findOne({}, (err, data) => {
      if (!err) {
        console.log(data); //文档对象
        console.log(data.id);  //5b2b4c467cbf8e2ef0040131
        console.log(data._id); //5b2b4c467cbf8e2ef0040131
        console.log(data.name); //周鑫
        /*data.age = 20;
        data.save(err => {
          if (!err) console.log('文档插入成功~~~');
          else console.log(err);
        });*/
        /*data.update({$set: {address: '深圳'}}, err => {
